...
BOOST_FORCEINLINE float CalcAoiDistSquare(const SquarePos &a, const SquarePos &b) {
  float dx = a.x - b.x;
  float dz = a.z - b.z;
  return dx * dx + dz * dz;
}
...
void SquareAoi::_RemoveFromSquare(Nuid nuid, SquarePlayerAoi* pptr) {
  ...
  if (square_ptr != squares_.end()) {
	...
	if (pptr->square_index >= square.size())
	{
...
void SquareAoi::_AddToSquare(Nuid nuid, SquarePlayerAoi* pptr) {
...
void SquareAoi::AddPlayer(Nuid nuid, float x, float y, float z) {
  ...
  if (piter != player_map_.end()) {
    ...
    auto ret = player_map_.emplace(nuid, new SquarePlayerAoi(nuid, x, y, z));
    if (ret.second) {
...
SquareAoiUpdateInfo SquareAoi::_UpdatePlayerAoi(Uint32 cur_aoi_map_idx,
                                          SquarePlayerAoi* pptr) {
  SquareAoiUpdateInfo aoi_update_info;
  ...
  for (auto& sensor : pptr->sensors) {
    ...
    SquareSensorUpdateInfo update_info;
...
void SquareAoi::_CalcAoiPlayers(const SquarePlayerAoi& player, const SquareSensor& sensor,
  ...
  size_t max_num = 0;
  _GetSquaresAndPlayerNum(player.pos, radius, &check_squares, &max_num, player_map_.size());
...
void SquareAoi::_CheckLeave(SquarePlayerAoi* pptr, float radius_square,
...
void SquareAoi::_CheckEnter(SquarePlayerAoi* pptr, float radius_square,
